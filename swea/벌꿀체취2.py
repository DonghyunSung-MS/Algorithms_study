...
def solve(test_case):
    ...
    for i in range(N):
        for j in range(N-M+1):
            ...
            # 2 2 5 2^2 + 2^2 < 5^2
            honey = take_honey(i, j, board)

            possible_honeys.append((i, j, honey))


    for a, b in nCc(possible_honeys, 2):
        # check a , b 충돌
        ai, aj, ahoney = a
        bi, bj, bhoney = b

        cond = (ai==bi) and (aj<=bj<aj+M or bj<=aj<bj+M)
        tmp_honey_sum = ahoney + bhoney
        if not cond and tmp_honey_sum > max_honey_total:
            max_honey_total = tmp_honey_sum
